/* global React, AUTH_TEXT, BrandMark, GoogleIcon, EyeIcon, pwdScore */
...
function PCLogin({ lang = "ko", dark = false, onSwitch }) {
  ...
  const [showPwd, setShowPwd] = useStateP(false);
  const [remember, setRemember] = useStateP(true);
  const ink = dark ? "#fff" : "#1a1814";
...
function PCSignup({ lang = "ko", dark = false, onSwitch }) {
  ...
  const [email, setEmail] = useStateP("");
  const [pwd, setPwd] = useStateP("");
  const [agree, setAgree] = useStateP(false);
  const [marketing, setMarketing] = useStateP(false);
  const [showPwd, setShowPwd] = useStateP(false);
  const ink = dark ? "#fff" : "#1a1814";
  const mute = dark ? "rgba(255,255,255,0.6)" : "rgba(26,24,20,0.6)";
  const subtle = dark ? "rgba(255,255,255,0.4)" : "rgba(26,24,20,0.42)";
...
function PCOnboarding({ lang = "ko", dark = false, initialStep = 0 }) {
  ...
  const [purpose, setPurpose] = useStateP(2);
  const [salary, setSalary] = useStateP(25);
  const [picks, setPicks] = useStateP([0, 1, 2, 4]);
  const ink = dark ? "#fff" : "#1a1814";
  const mute = dark ? "rgba(255,255,255,0.6)" : "rgba(26,24,20,0.6)";
  const yellow = "#ffd84d";
...
window.PCOnboarding = PCOnboarding;
