...
const TWEAK_DEFAULTS: Tweaks = /*EDITMODE-BEGIN*/ {
  ...
  density: 'comfy',
  showCalendar: true,
  // 인증/디바이스 미리보기 (Supabase 연결 전 임시 토글)
  authed: true,
  forceMobile: false,
  authPreview: 'login', // login | signup | onboarding | forgot
} /*EDITMODE-END*/;
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakSection title="인증 (mock — Supabase 연결 전)">
          <TweakToggle
            label="로그인 상태"
            value={tweaks.authed}
            onChange={(v: boolean) => setTweak('authed', v)}
          />
          <TweakRadio
            label="인증 화면"
    ...
    <>
      ...
      {modal?.type === 'txn' && (
        <TxnModal
          ...
          onDelete={(t: any) => console.log('delete txn', t)}
          onSave={(t: any) => console.log('save txn', t)}
        />
      )}
      {modal?.type === 'event' && (
        <EventModal
          onClose={closeModal}
          editing={modal.editing}
          onDelete={(e: any) => console.log('delete event', e)}
          onSave={(e: any) => console.log('save event', e)}
        />
      )}
