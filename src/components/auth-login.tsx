...
const AUTH_TEXT = {
  ko: {
    ...
    cats: ["식비", "교통", "카페", "쇼핑", "구독", "운동", "취미", "여행", "선물", "공과금", "병원", "기타"],
    // step 4
    ob4Title: "준비됐어요!",
    ...
    fpSentSub: "받은 편지함에서 이메일을 확인하고 링크를 눌러 비밀번호를 다시 만들어주세요",
    fpResend: "다시 보내기",
  ...
  en: {
    ...
    cats: ["Food", "Transit", "Café", "Shopping", "Subscriptions", "Fitness", "Hobbies", "Travel", "Gifts", "Bills", "Health", "Other"],
    ob4Title: "All set!",
    ...
    fpSentSub: "We sent a link to reset your password. Tap it from your email to continue",
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{
        ...
        <div style={{ width: size * 0.42, height: size * 0.42, borderRadius: "50%", background: ink }} />
        <div style={{ position: "absolute", top: size * 0.18, right: size * 0.2, width: size * 0.22, height: size * 0.22, borderRadius: "50%", background: ink }} />
      </div>
      <b style={{ fontSize: size * 0.55, fontWeight: 800, letterSpacing: "-0.02em", color: ink }}>Dayflow</b>
    </div>
  );
}

// ─────────── Input field ───────────
function Field({ label, type = "text", value, onChange, dark, rightSlot, autoFocus, placeholder, error }) {
  const ink = dark ? "#fff" : "#1a1814";
  const mute = dark ? "rgba(255,255,255,0.55)" : "rgba(26,24,20,0.55)";
  const bg = dark ? "rgba(255,255,255,0.06)" : "#fff";
  const line = error ? "#dc4c3e" : (dark ? "rgba(255,255,255,0.14)" : "rgba(0,0,0,0.1)");
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6, width: "100%" }}>
      {label && <label style={{ fontSize: 12, fontWeight: 600, color: mute, letterSpacing: "0.01em" }}>{label}</label>}
      <div style={{
        position: "relative",
        background: bg,
      ...
      {error && <span style={{ fontSize: 11, color: "#dc4c3e", fontWeight: 500 }}>{error}</span>}
    </div>
  );
}

// ─────────── Button (primary / google / ghost) ───────────
function Btn({ children, kind = "primary", onClick, dark, disabled, full = true }) {
  const styles = {
    primary: {
...
const GoogleIcon = () => (
  <svg width="17" height="17" viewBox="0 0 18 18">
    ...
    <path fill="#FBBC04" d="M3.97 10.71A5.4 5.4 0 0 1 3.68 9c0-.59.1-1.17.29-1.71V4.96H.96a9 9 0 0 0 0 8.08l3.01-2.33z"/>
...
const EyeIcon = ({ on, dark }) => (
  <button type="button" style={{ background: "transparent", border: "none", cursor: "pointer", padding: 6, color: dark ? "rgba(255,255,255,0.55)" : "rgba(0,0,0,0.5)" }}>
    {on ? (
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: "center" }}>
          <h1 style={{ fontSize: 26, fontWeight: 800, letterSpacing: "-0.02em", margin: "8px 0 8px" }}>{t.loginTitle}</h1>
          <p style={{ fontSize: 13, color: mute, margin: 0, lineHeight: 1.5 }}>{t.loginSub}</p>
        </div>

        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <Field label={t.email} type="email" value={email} onChange={(e) => setEmail(e.target.value)}
            placeholder="you@example.com" dark={dark} />
          <Field
            label={t.password}
        ...
        <div style={{ marginTop: "auto", textAlign: "center", paddingTop: 24 }}>
          <span style={{ fontSize: 13, color: mute }}>
            {t.noAccount}{" "}
            <a onClick={() => onSwitch && onSwitch("signup")} style={{ color: ink, fontWeight: 700, textDecoration: "none", cursor: "pointer" }}>{t.signup} →</a>
          </span>
        </div>
      </div>
    );
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ position: "absolute", bottom: -50, left: -20, width: 100, height: 100, borderRadius: "50%", background: "rgba(0,0,0,0.06)" }} />
          <div style={{ position: "relative" }}>
            <BrandMark size={28} />
            <h1 style={{ fontSize: 30, fontWeight: 800, letterSpacing: "-0.025em", margin: "28px 0 8px", lineHeight: 1.1 }}>{t.loginTitle}</h1>
            <p style={{ fontSize: 13, margin: 0, opacity: 0.7, lineHeight: 1.5 }}>{t.tagline}</p>
          </div>
        </div>

        <div style={{ padding: "28px 24px 32px", display: "flex", flexDirection: "column", gap: 18, flex: 1 }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ marginTop: 56 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: mute, letterSpacing: "0.1em", textTransform: "uppercase" }}>{lang === "ko" ? "로그인" : "Sign in"}</div>
        <h1 style={{ fontSize: 32, fontWeight: 800, letterSpacing: "-0.03em", margin: "8px 0 12px", lineHeight: 1.1 }}>{t.loginTitle}</h1>
      ...
      <div style={{ marginTop: "auto", textAlign: "center", paddingTop: 32, display: "flex", flexDirection: "column", gap: 6 }}>
        <a href="#" style={{ fontSize: 12, fontWeight: 600, color: mute, textDecoration: "none" }}>{t.forgot}</a>
        <span style={{ fontSize: 13, color: mute }}>
          {t.noAccount}{" "}
          <a onClick={() => onSwitch && onSwitch("signup")} style={{ color: ink, fontWeight: 700, textDecoration: "none", cursor: "pointer" }}>{t.signup}</a>
...
export { LoginScreen, AUTH_TEXT, BrandMark, Field, Btn, GoogleIcon, EyeIcon, pwdScore };