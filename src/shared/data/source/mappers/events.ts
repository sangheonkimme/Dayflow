...
export function toDomain(row: Row): CalendarEvent {
  ...
  const startTime = row.starts_at.length >= 16 ? row.starts_at.slice(11, 16) : undefined;
  const endTime = row.ends_at && row.ends_at.length >= 16 ? row.ends_at.slice(11, 16) : undefined;
  return {
    id: row.id,
    title: row.title,
    date,
...
export function toRow(input: Partial<CalendarEvent>, userId: string): Insert {
  ...
  const ends_at = input.endTime ? new Date(`${date}T${input.endTime}:00`).toISOString() : null;
  return {
